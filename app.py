...
# --- 3. MOTOR DE COMENTARIOS (PROTEGIDO) ---
...
def parse_ocr_data(ocr_text):
    ...
    for line in ocr_text.strip().split('\n'):
        ...
        if len(p) >= 6:
            ...
            if any(key in n_d for key in keywords) or any(key in n_l for key in keywords): continue
...
def actualizar_excel_maestro(archivo_maestro, df_actual, num_sem):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        for hoja in xls.sheet_names:
            ...
            if col_dato:
                if col_target not in df_h.columns: df_h[col_target] = '00:00:00' if hoja != 'CV' else 'NC'
...
if st.button("🚀 PROCESAR Y GENERAR REPORTE"):
    if raw_data.strip() and ocr_input.strip() and archivo_maestro:
        ...
        col1.download_button("📄 DESCARGAR WORD (Bloqueado)", generar_word(df_semana, sem_num, dist_p, larg_p), f"Reporte_TYM_Sem_{sem_num}.docx")
